...
Ext.define('CMS.mask', {
    statics: {
        getGridValue: function (v, column) {
            if (v.length != column.mask.viewMask.length) {
                var value = column.mask.viewMask;

                for (var i = 0; i < v.toString().length; i++) {
                    value = value.replace('_', v[v[i]]);
                };

                if (value.length == column.mask.rawMask.length) {
                    v = value;
                };
            };
        ...
        returnMask: function (mask) {
            ...
            for (var i = 0; i < mask.length; i++) {
                if (regexp) {
                    ...
                    viewMask += '_';
                    maskArray[mai] = mask.charAt(i);
                    mai++;
                } else {
                    viewMask += mask.charAt(i);
                    maskArray[mai] = RegExp.escape(mask.charAt(i));
                    mai++;
                };
            };

            specialChars = viewMask.replace(/(L|l|9|A|_|X)/g, '');
    ...
    constructor: function (mask, clearWhenInvalid) {
        ...
        var ret = CMS.mask.returnMask(mask);

        if (ret) {
            this.rawMask = ret.mask;
            this.viewMask = ret.viewMask;
            this.maskArray = ret.maskArray;
            this.specialChars = ret.specialChars;
        }
    ...
    getValue: function (v, field) {
        if (field.saveMask === true) {
            return v;
        };

        var chars = this.specialChars + '_';

        for (var i = 0; i < chars.length; i++) {
            var p = chars[i];

            while (v.toString().indexOf(p) != -1) {
                v = v.toString().replace(p, '');
            };
        };

        return v == this.viewMask ? '' : v;
    },
    removeMask: function () {
        var maskPlugin = this;
        ...
        field.inputEl.un('keypress', maskPlugin.processKeyPress, maskPlugin);
        field.inputEl.un('keydown', maskPlugin.processKeyDown, maskPlugin);

        if (Ext.isSafari || Ext.isIE) {
            field.inputEl.un('paste', maskPlugin.startTask, maskPlugin);
            field.inputEl.un('cut', maskPlugin.startTask, maskPlugin);
        }

        if (Ext.isGecko || Ext.isOpera) {
            field.inputEl.un('mousedown', maskPlugin.setPreviousValue, maskPlugin);
        }

        if (Ext.isGecko) {
            field.inputEl.un('input', maskPlugin.onInput, maskPlugin);
        }

        if (Ext.isOpera) {
            field.inputEl.un('input', maskPlugin.onInputOpera, maskPlugin);
        }

        if (Ext.isArray(field.plugins)) {
            Ext.Array.remove(field.plugins, maskPlugin)
        }
        else {
            field.plugins = null;
        }
    ...
    isValid: function () {
        return !(this.inputTextElement.value.indexOf('_') > -1)
    }
});
